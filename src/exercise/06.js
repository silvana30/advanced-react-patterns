...
function useControlSwitchWarning(controlPropValue, controlPropName = '', componentName = '') {
  const isControlled = controlPropValue != null
  const {current: wasController} = useRef(isControlled)

  useEffect(() => {
    warning(!(isControlled && !wasController),
...
function useOnChangeReadOnlyWarning(controlPropValue, controlPropName, componentName, hasOnChange, readOnly, readOnlyProp, initialValueProp, onChangeProp) {
  const isControlled = controlPropValue != null
  useEffect(() => {
    warning(!(!hasOnChange && isControlled && !readOnly),
      `Warning: Failed prop type: You provided a ${controlPropValue} prop to a form field without an ${onChangeProp} handler. This will render a read-only field. If the field should be mutable use ${initialValueProp}. Otherwise, set either ${onChangeProp} or ${readOnlyProp}.`)
  }, [hasOnChange, isControlled, readOnly, controlPropValue, onChangeProp, initialValueProp, readOnlyProp])
}

function useToggle({
                     initialOn = false,
                     reducer = toggleReducer, onChange, on: controlledOn, readOnly,
                   } = {}) {
